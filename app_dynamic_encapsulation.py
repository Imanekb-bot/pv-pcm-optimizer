...
# Le gain et la couverture de l'architecture encapsulée sont lus directement
# depuis le fichier d'optimisation, pour varier selon la ville et la saison.
...
def compute_values(best_100, best_enc, price_kwh):
    """
    Calculs technico-économiques et environnementaux.

    Principe :
    - PV seul : référence énergétique issue du modèle.
    - PV--PCM 100 % : résultats issus du fichier classique.
    - PV--PCM encapsulé : résultats issus du fichier d'optimisation encapsulée.

    Important :
    le gain de l'encapsulation n'est pas fixé à 3 %. Il est calculé à partir
    de l'énergie simulée pour la configuration encapsulée optimale.
    """

    # Energies journalières issues des simulations
    e_pv_day = float(best_100["E_PV_seul_kWh_m2_day"])
    e_100_day = float(best_100["E_PV_PCM_kWh_m2_day"])
    e_enc_day = float(best_enc["E_architecture_kWh_m2_day"])

    # Energies annuelles
    e_pv_year = e_pv_day * 365.0
    e_100_year = e_100_day * 365.0
    e_enc_year = e_enc_day * 365.0

    # Gains énergétiques
    gain_100_pct = (e_100_year / e_pv_year - 1.0) * 100.0
    gain_enc_vs_pv_pct = (e_enc_year / e_pv_year - 1.0) * 100.0
    gain_enc_vs_100_pct = (e_enc_year / e_100_year - 1.0) * 100.0
    ...
    # Couverture optimisée réelle lue depuis Excel
    coverage_enc = float(best_enc["Coverage"])
    mass_enc = PCM_MASS_100 * coverage_enc
...
with c3:
    st.markdown(f"""
    <div class="card">
    <div class="metric-title">Amélioration vs PCM 100 %</div>
...
with colB:
    ...
    <p>La couverture affichée est issue directement de l'optimisation paramétrique.</p>
    </div>
    """, unsafe_allow_html=True)
...
with k3:
    st.markdown(f"""
    <div class="key-box">Amélioration vs PCM 100 % : {vals['gain_enc_vs_100_pct']:.1f} %</div>
    """, unsafe_allow_html=True)
