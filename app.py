...
# Gain supplementaire de l'architecture encapsulee par rapport au PV--PCM 100 %.
# Il varie selon la ville et la saison afin de rester coherent avec les conditions climatiques.
ENCAPSULATION_EXTRA_GAIN_DEFAULT = 2.50


# Gains supplementaires (%) imposes pour le PCM encapsule par rapport au PCM 100 %.
# Valeurs comprises entre 2.10 % et 3.00 %, plus elevees pour les climats chauds et fortement ensoleilles.
ENCAPSULATION_EXTRA_GAIN_MAP = {
    "Rabat": {"Mars": 2.30, "Juin": 2.60, "Septembre": 2.50, "Decembre": 2.10, "Décembre": 2.10},
    "Marrakech": {"Mars": 2.50, "Juin": 2.90, "Septembre": 3.00, "Decembre": 2.30, "Décembre": 2.30},
    "Ifrane": {"Mars": 2.10, "Juin": 2.40, "Septembre": 2.50, "Decembre": 2.10, "Décembre": 2.10},
    "Ouarzazate": {"Mars": 2.60, "Juin": 3.00, "Septembre": 2.90, "Decembre": 2.40, "Décembre": 2.40},
    "Merzouga": {"Mars": 2.70, "Juin": 3.00, "Septembre": 2.90, "Decembre": 2.50, "Décembre": 2.50},
}

def get_encapsulation_extra_gain(ville, saison):
    ville = str(ville)
    saison = str(saison)
    return ENCAPSULATION_EXTRA_GAIN_MAP.get(ville, {}).get(
        saison, ENCAPSULATION_EXTRA_GAIN_DEFAULT
    )
...
def compute_values(best_100, best_enc, price_kwh, extra_gain_pct):
    ...
    # Gain supplementaire impose selon la ville et la saison.
    gain_enc_vs_100_pct = float(extra_gain_pct)
...
extra_gain_pct = get_encapsulation_extra_gain(ville, saison)
vals = compute_values(best_100, best_enc, prix_kwh, extra_gain_pct)
...
with c3:
    ...
    <div class="metric-sub">Par rapport au PCM 100 %</div>
    </div>
    """, unsafe_allow_html=True)
with c4:
    st.markdown(f"""
...
# =========================================================
# RESULTATS PAR MODULE SELON LA PUISSANCE NOMINALE
# =========================================================
st.markdown("<div class='section'>Résultats par module sélectionné</div>", unsafe_allow_html=True)

module_vals = {
    "E_PV_module": vals["e_pv_year"] * surface_module,
    "E_100_module": vals["e_100_year"] * surface_module,
    "E_enc_module": vals["e_enc_year"] * surface_module,
    "R_PV_module": vals["r_pv"] * surface_module,
    "R_100_module": vals["r_100"] * surface_module,
    "R_enc_module": vals["r_enc"] * surface_module,
    "CO2_PV_module": vals["co2_pv"] * surface_module,
    "CO2_100_module": vals["co2_100"] * surface_module,
    "CO2_enc_module": vals["co2_enc"] * surface_module,
    "CAPEX_100_module": vals["capex_100"] * surface_module,
    "CAPEX_enc_module": vals["capex_enc"] * surface_module,
    "OPEX_100_module": vals["opex_100"] * surface_module,
    "OPEX_enc_module": vals["opex_enc"] * surface_module,
    "Net_100_module": vals["net_100"] * surface_module,
    "Net_enc_module": vals["net_enc"] * surface_module,
}

m1, m2, m3, m4 = st.columns(4)
with m1:
    st.metric("Surface du module", f"{surface_module:.2f} m²")
with m2:
    st.metric("Énergie PV seul", f"{module_vals['E_PV_module']:.1f} kWh/an")
with m3:
    st.metric("Énergie PV--PCM encapsulé", f"{module_vals['E_enc_module']:.1f} kWh/an")
with m4:
    st.metric("Revenu encapsulé", f"{module_vals['R_enc_module']:.1f} DH/an")
...
module_table = pd.DataFrame({
    "Indicateur": [
        ...
        "Revenu annuel (DH/an)",
        "CO₂ évité (kgCO₂/an)",
        "CAPEX additionnel (DH)",
        "OPEX annuel (DH/an)",
        "Gain net annuel (DH/an)",
        "Temps de retour simple",
    ],
    "PV seul": [
        f"{module_vals['E_PV_module']:.2f}",
        f"{module_vals['R_PV_module']:.2f}",
        f"{module_vals['CO2_PV_module']:.2f}",
        "0",
        "0",
        "--",
        "--",
    ...
    "PV--PCM 100 %": [
        f"{module_vals['E_100_module']:.2f}",
        f"{module_vals['R_100_module']:.2f}",
        f"{module_vals['CO2_100_module']:.2f}",
        f"{module_vals['CAPEX_100_module']:.2f}",
        f"{module_vals['OPEX_100_module']:.2f}",
        f"{module_vals['Net_100_module']:.2f}",
        f"{vals['tri_100']:.2f} ans",
    ],
    "PV--PCM encapsulé": [
        f"{module_vals['E_enc_module']:.2f}",
        f"{module_vals['R_enc_module']:.2f}",
        f"{module_vals['CO2_enc_module']:.2f}",
        f"{module_vals['CAPEX_enc_module']:.2f}",
        f"{module_vals['OPEX_enc_module']:.2f}",
        f"{module_vals['Net_enc_module']:.2f}",
...
Energie PV seul : {module_vals['E_PV_module']:.2f} kWh/an
Energie PCM 100 % : {module_vals['E_100_module']:.2f} kWh/an
Energie encapsule : {module_vals['E_enc_module']:.2f} kWh/an
Revenu PV seul : {module_vals['R_PV_module']:.2f} DH/an
Revenu PCM 100 % : {module_vals['R_100_module']:.2f} DH/an
Revenu encapsule : {module_vals['R_enc_module']:.2f} DH/an
CAPEX PCM 100 % : {module_vals['CAPEX_100_module']:.2f} DH
CAPEX encapsule : {module_vals['CAPEX_enc_module']:.2f} DH
CO2 evite encapsule : {module_vals['CO2_enc_module']:.2f} kgCO2/an
"""
